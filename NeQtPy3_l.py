...
from PyQt5.QtWidgets import QWidget, QStackedWidget, QApplication,\
 ...
 QGroupBox, QFormLayout, QSpinBox, QPushButton, QTextEdit
...
class Window(QMainWindow):
    ...
    def initUI(self):
        ...
        expAction = QAction('&Load Experimental Data',self)
        expAction.setStatusTip('File to be fitted')
        expAction.triggered.connect(self.exp)
        losAction = QAction('&Generate LOS.dat File', self)
...
class Widgettown(QWidget):  # WHERE ALL OF THE FUNCTIONALITY IS LOCATED
    ...
    def stack2UI(self):  # THE LOS DATA CAPABILITY
        ...
        layout3.addRow(self.simplexText)
    ...
    def simplex(self):
        ...
        # printerino("exp_temp.out", "100", "100", "1e-6", self.los)
        name = genLOSFile(self, self.los['Temperature'].text(), self.los['SpecNum'].value(), self.los['Species'])
        print(name)
        path = folderMake(self)
        print(path)
        moveNEQAIRFiles(self)
        submitFile(self)
        simplexerino(self)
        # command = "python3"
        # args = ["specfit.py"]
        ...
        # self.process.finished.connect(self.reEnableSimplex)
    ...
    def reEnableSimplex(self):
        self.simplexButton.setEnabled(1)
        self.simplexButton.setText("Again?")

    def dataReady(self):
        cursor = self.simplexText.textCursor()
        cursor.movePosition(cursor.End)
        so = self.process.readAllStandardOutput()  
        cursor.insertText(str(so))
    ...
    def readplot(self):  # OPENING AND READING FIGURE IN
        ...
        if datafile:
            self.exp_datafile = str(datafile)
            try:
                self.plotfile =np.genfromtxt(self.exp_datafile, comments='#', skip_header=3, names=True, unpack=True, autostrip=True)
                ...
                self.colchange1.setCurrentIndex(0)
                self.colchange2.setCurrentIndex(1)
                i1 = self.colchange1.currentIndex()
                # i1max = max(self.plotfile[self.plotfile.dtype.names[i1]])
                i2 = self.colchange2.currentIndex()
                i2max = max(self.plotfile[self.plotfile.dtype.names[i2]])
                i1n = self.plotfile[self.plotfile.dtype.names[i1]]
                #  / i1max
                i2n = self.plotfile[self.plotfile.dtype.names[i2]] / i2max
                sax.plot(i1n, i2n, '-')
                self.canvas.draw()
            except:
                self.plotfile =np.genfromtxt(self.exp_datafile, dtype=float, comments="(", skip_header=1, names=True, unpack=True)
                sax = self.figure.add_subplot(111)  # adds figure as a subplot. might be nice to use this to allow user to plot multiple things.
